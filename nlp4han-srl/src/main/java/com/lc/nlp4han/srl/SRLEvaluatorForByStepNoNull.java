...
/**
 * 基于分步训练模型的评估类(不包含NULL标签)
 * @author 王馨苇
 *
 */
public class SRLEvaluatorForByStepNoNull extends Evaluator<SRLSample<HeadTreeNode>>{

	private SRLMEForIdentification taggerIden;
	private SRLMEForClassificationNoNull taggerClas;
	private SRLMeasure measure;
	
	public SRLEvaluatorForByStepNoNull(SRLMEForIdentification taggerIden, SRLMEForClassificationNoNull taggerClas) {
		this.taggerIden = taggerIden;
		this.taggerClas = taggerClas;
	}
	
	public SRLEvaluatorForByStepNoNull(SRLMEForIdentification taggerIden, SRLMEForClassificationNoNull taggerClas, SRLEvaluateMonitor... evaluateMonitors) {
