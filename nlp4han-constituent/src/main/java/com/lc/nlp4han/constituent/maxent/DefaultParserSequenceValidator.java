...
public class DefaultParserSequenceValidator implements ParserSequenceValidator<HeadTreeNode>
{
	...
	public boolean validSequenceForChunk(int i, List<HeadTreeNode> posTree, List<String> outcomes, String out)
	{
		...
		else if (i == posTree.size() - 1)
		{
			...
			{
				if (outcomes.get(i - 1).split("_")[0].equals("join") || outcomes.get(i - 1).equals("other")
						|| outcomes.get(i - 1).split("_")[0].equals("start"))
				{
					return true;
				}
			}
			...
			{
				if (outcomes.get(i - 1).split("_")[0].equals("start")
						|| outcomes.get(i - 1).split("_")[0].equals("join"))
				{
	...
	public boolean validSequenceForBuildAndCheck(int i, List<HeadTreeNode> combineChunkTree, String out)
	{
		...
		else
		{
			...
			{
				if (combineChunkTree.get(i - 1).getNodeNameLeftPart().equals("start")
						|| combineChunkTree.get(i - 1).getNodeNameLeftPart().equals("join"))
				{
